...
public:		// Load, Save and Generate Subsets 
	...
	string  SubDir()	const	{return(string("_"+Set::GetName()));};
protected:
	Vec<Set>	m_train;
	Vec<Set>	m_blind;
};
...
template <class Set> void CVSet<Set>::GenerateSubsets(unsigned subnum)
{
	...
	for(unsigned cv=0; cv<subnum; cv++)
	{
		...
		{
			if((i % subnum)==cv)
				blind_members.push_back(ord[i]);
			else
				train_members.push_back(ord[i]);
		}
		m_blind[cv].CreateSubset(*this,blind_members);
		m_train[cv].CreateSubset(*this,train_members);
	}
}
