...
double	CMinimizerBase::MinimizeConjugateGradient(CNumVec &param, const double min_precision, unsigned restart_repeats)
{
	
	// First move broadly towards the minimum:
//	AdjustParameters(param);
	CNumVec new_direction;
	double last_dist=Gradient(param,new_direction);
	double norm=new_direction.ScalarProduct(new_direction);
	if(norm==0.0)
		return(last_dist);
	double normalization= -sqrt(min_precision/norm);
...
//		AdjustParameters(param);
		...
		{
			last_dist=Gradient(param,new_direction);
			new_direction*=-1;
		}
		else
		{
		...
		if(norm==0.0)
			return(last_dist);
		normalization= sqrt(min_precision/norm);
	...
	while(last_dist-new_dist>min_precision || count<1);
//	exit(0);
	return(new_dist);

}

double	CMinimizerBase::MinimizeSteepestDescentOneDimensional(CNumVec &param, const double min_precision)
{
	assert(param.size()==1);
	CNumVec gradient;
	double start_dist=Gradient(param,gradient);
	double gradient_norm=gradient.ScalarProduct(gradient);
	if(gradient_norm==0.0)
		return(start_dist);
	double normalization= -sqrt(min_precision/gradient_norm);
	gradient*=normalization;
	double dist = LineMinimizer(param,start_dist,gradient,false);
	return(dist);
}



double	CMinimizerBase::MinimizeSteepestDescent(CNumVec &param, const double min_precision)
{
	unsigned count=0;
...
//		_log << endl;
	...
	while(last_dist-new_dist>min_precision || count<2); //  || (count<2 && param.size()>1));
//	exit(0);
	return(new_dist);
}
...
void CMinimizerBase::TestGradient(const CNumVec & param, double delta)
{
	...
	CNumVec gradient;
	double dist0=Gradient(param,gradient);
