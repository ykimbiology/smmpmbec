...
void CSeqSet::Load(const string & name, bool allEqual )
{
	ex_ifstream in(name);
	string x;
	in >> x >> m_seq_length;
	...
	try
	{
		...
		// If the <m_inequalities> should only contain equal signs, then do it here. YK.
		if (allEqual == true) {
			// DEBUG:
			cout << "allEqual = true\n";

			unsigned len = m_inequalities.size();
			m_inequalities.clear();
